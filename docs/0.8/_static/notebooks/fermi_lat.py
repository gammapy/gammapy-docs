...
# 
# To use Fermi-LAT data with Gammapy, you first have to use the Fermi ST to prepare an event list (using ``gtselect`` and ``gtmktime``, exposure cube (using ``gtexpcube2`` and PSF (using ``gtpsf``). You can then use [gammapy.data.EventList](http://docs.gammapy.org/0.8/api/gammapy.data.EventList.html), [gammapy.maps](http://docs.gammapy.org/0.8/maps/index.html) and the [gammapy.irf.EnergyDependentTablePSF](http://docs.gammapy.org/0.8/api/gammapy.irf.EnergyDependentTablePSF.html) to read the Fermi-LAT maps and PSF, i.e. support for these high-level analysis products from the Fermi ST is built in. To do a 3D map analyis, you can use [MapFit](http://docs.gammapy.org/0.8/api/gammapy.cube.MapFit.html) for Fermi-LAT data in the same way that it's use for IACT data. This is illustrated in this notebook. A 1D region-based spectral analysis is also possible, this will be illustrated in a future tutorial. There you have to extract 1D counts, exposure and background vectors and then pass them to [SpectrumFit](http://docs.gammapy.org/0.8/api/gammapy.spectrum.SpectrumFit.html).
# 
# ## Setup
# 
...
# ## Events
# 
# To load up the Fermi-LAT event list, use the [gammapy.data.EventList](http://docs.gammapy.org/0.8/api/gammapy.data.EventList.html) class:
...
# 
# To load the isotropic diffuse model with Gammapy, use the [gammapy.spectrum.models.TableModel](http://docs.gammapy.org/0.8/api/gammapy.spectrum.models.TableModel.html). We are using `'fill_value': 'extrapolate'` to extrapolate the model above 500 GeV:
