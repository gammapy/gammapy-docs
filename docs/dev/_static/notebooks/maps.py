...
counts_3d = WcsNDMap.create(binsz=0.1, width=10.0, skydir=(0, 0),
...
# For 3d geometries this operation can be performed directly using the `.interp_to_geom()` method:

# In[ ]:


#create new geometry
energy_axis = MapAxis.from_bounds(10., 2E3, 6, interp='log', name='energy_true', unit='GeV')
skydir = SkyCoord(266.4, -28.9, frame="icrs", unit="deg")
...
m_iem_interp = m_iem_gc.interp_to_geom(wcs_geom_3d, preserve_counts=False, method="linear", fill_value=np.nan)
print(m_iem_interp)
